...
Getting back to the framework of mixture models, from the stick-breaking representation it is straightforward to extend the finite mixtures described in the previous section to nonparametric mixtures: following the structure of equation (\ref{eq:ch1_finitemix_h2}), DP mixtures (DPM) can be written as
\begin{equation*}
\begin{gathered}
G \sim \DP(\alpha,G_0)\\
\theta_i \mid G \sim G \\
...
\end{gathered}
\end{equation*}
Differently from finite mixtures, DPM are an infinite mixture model, as they assume a countably infinite number of components. 
