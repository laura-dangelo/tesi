...
In particular, the proposed sampling strategies would be straightforward to apply under a data augmentation scheme where latent allocation variables indicate if the observation comes from the spike in zero or from the Poisson component. 

Considering the simplest setting where for all $i=1,\dots,n$, $\pi_i = \pi \sim \mathrm{Beta}(\alpha,\gamma)$, we can introduce the variables $v_i\in\{0,1\}$, with $v_i = 1$ indicating the assignment to the degenerate component at zero. Then, conditionally on the observed value of $y_i$, the distribution of $v_i$ is Bernoulli with conditional probabilities
\begin{equation}
\begin{gathered}
