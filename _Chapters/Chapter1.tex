...
At rest, most neurons have an intracellular calcium concentration of about 100 nm; however, during electrical activity, the concentration can rise transiently up to levels around 1000 nm~\parencite{berridge2000}. 
...
The development of calcium imaging techniques involved two parallel processes: the development of calcium indicators, which are fluorescent molecules that react when binding to the calcium ions, and the implementation of the appropriate imaging instrumentation, in particular, the introduction of two-photon microscopy~\parencite{denk1990}.
In recent years, the innovation achieved in these two fields has allowed for real-time observation of biological processes at the single-cell level simultaneously for large groups of neurons~\parencite{grienberger2012}. 

The output of two-photon calcium imaging is a movie of time-varying fluorescence intensities, and a first complex pre-processing phase deals with the identification of the spatial location of each neuron in the optical field and source extraction~\parencite{mukamel2009,dombeck2010}. The resulting processed data consist of a fluorescent calcium trace for each observable neuron in the targeted area which, however, is only a proxy of the underlying neuronal activity.
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{ch1_neuron2_trace}
	\centering
	\includegraphics[width=\textwidth]{ch1_neuron1_trace}
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{ch1_example_data_hippoc}
...
The dataset we consider collects the activity of 325 neurons located in the hippocampus of a mouse during navigation in a closed environment for about 12 minutes. The calcium level is recorded with a frequency of 15 frames per second, resulting in 10870 calcium measurements over time for each series.
...
Denoting with $\{\theta^*_1, \dots, \theta^*_{K^+}\}$ the unique values among the $\theta_i$, and letting $n_k$ be the number of $\theta_i$ equal to $\theta^*_k$, for $i=1,\dots,n$ and $k=1,\dots,K^+$, the predictive distribution can be written as
\begin{equation*}
...
data a priori: in the following, we will define \textit{clusters} these non-empty components. In general, in a sample of size $n$, the prior expected number of clusters $\mathbb{E}(K^+)$ is approximately equal to $\alpha \log(1 + n/\alpha)$.
...
The slice sampler \parencite{walker2007, kalli2011} has been adopted as an alternative ``dynamic'' truncation method to automatically select the active components of the mixture. 
...
\subsection{Finite mixtures with an unknown number of components}
...
The hierarchical Dirichlet process~\parencite{Teh2006} arises from the desire to flexibly model the distributions of the observations of each group, while also performing clustering to capture latent structures among all individuals.
...
To overcome this limitation,~\textcite{rodriguez2008} introduced the nested Dirichlet process, which allows to obtain a clustering both of the observations within each group, and of the groups themselves. 
...
In a recent paper by~\textcite{camerlenghi2019}, however, they noted a degeneracy property that occurs in the nested DP in case of  ties across samples at the observed or latent level.
