...
We considered scenarios with varying sample size ($n = 20, 30$) and length of the calcium traces ($T = 100, 200$). Here we only present one simulated scenario for each setting to briefly describe the model's behavior and assess its performance. To compare the results of the proposed method, we also apply a standard two-stage approach to the simulated data. Specifically, we first deconvolve the simulated calcium traces using the approach introduced by \textcite{jewell2019}, then, we cluster the extracted series of the signal using a hierarchical clustering based on the Hamming distance.
